...
{
	Super::BeginPlay();
	randColor = FLinearColor(UKismetMathLibrary::RandomFloatInRange(0.f, 1.f), UKismetMathLibrary::RandomFloatInRange(0.f, 1.f), 1.f);
...
void Afirstperson415Projectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (OtherActor != nullptr)
	{
		if (colorP)
		{
			...
			if (procTerrain)
			{
				procTerrain->AlterMesh(Hit.ImpactPoint); // alters the envorment based on where the projectile lands
			}
	}
}